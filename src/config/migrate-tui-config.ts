...
async function opencodeFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.KILO_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("spawnbot", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "spawnbot")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "spawnbot"))
  }
  if (Flag.KILO_CONFIG) files.push(Flag.KILO_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "spawnbot"))
