...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run spawnbot", // kilocode_change
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
        ...
        UI.println()
        UI.println("Starting spawnbot...")
        UI.println()

        // Launch spawnbot TUI with session ID if available
        const { spawn } = await import("child_process")
        const opencodeArgs = sessionId ? ["-s", sessionId] : []
        const opencodeProcess = spawn("spawnbot", opencodeArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })
